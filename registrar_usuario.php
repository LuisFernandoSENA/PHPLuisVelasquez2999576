...
<?php 
  if(isset($_POST["save"])){
    ...
    // Asegúrate que el campo en la base de datos sea 'documento' y 'user'
    $sql = $con->prepare("SELECT * FROM usuarios WHERE id_documento= '$doc' OR nombre= '$name'");
    $sql->execute();
    $fila = $sql->fetchAll(PDO::FETCH_ASSOC);

    if($fila){
      echo '<script>alert("EL DOCUMENTO O USUARIO EXISTE // CAMBIELOS//");</script>';
      echo "<script>location.href='registrar_usuario.php';</script>";

    }

    else 
    if ($doc=="" || $name=="" || $profesion=="" || $phone=="" || $email=="" || $contra=="" || $rol=="")
    {
      echo '<script>alert("EXISTE CAMPOS VACIOS");</script>';
      echo "<script>location.href='registrar_usuario.php';</script>";
    }

    else{
      $insertuser = $con->prepare("INSERT INTO usuarios (id_documento, nombre, nro_celular, email, profesion, contrasena, id_tipo_usuario) 
      VALUES ('$doc', '$name', '$phone', '$email', '$profesion', '$contra', '$rol')");
      $insertuser ->execute(); 
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <div class="form-outline mb-4">
                                    ...
                                    <select class="form-control" name="rol" id="rol">
                                        ...
                                        <?php 
                                        $consulta = $con ->prepare("SELECT * FROM tipo_usuario");
                                        $consulta->execute();
                                        while($pd = $consulta -> fetch(PDO::FETCH_ASSOC))
                                        {
                                            echo "<option value=" . $pd['id_tip_usuario'] . ">" . $pd['tipo_usuario'] . "</option>";
                                        }
                                        ?>
                                    </select>
                                </div>
